"""
MyTradeTool â Dashboard Streamlit V1.0
Architecture : Hetzner VPS (engine.py) â n8n webhook â Streamlit Cloud (ce fichier)
...
AUTO_REFRESH_SECONDS = 600  # 10 minutes (mÃªme cadence que l'engine)
...
def load_data():
    """RÃ©cupÃ¨re le dernier payload JSON depuis n8n."""
    try:
        resp = requests.get(N8N_WEBHOOK_URL, timeout=15)
        resp.raise_for_status()
        raw = resp.json()

        # n8n peut retourner un dict ou une liste â on normalise
...
def score_color(val):
    """Colorise les scores (rouge â orange â vert)."""
    try:
        v = float(val)
...
with st.sidebar:
    ...
    st.caption("V2.7 â Dashboard Streamlit")
    st.divider()

    page = st.radio(
        "Navigation",
        ["ð Screener", "ð¼ Portefeuille", "ð DÃ©tail Asset", "ð¨ Alertes / Signaux"],
        label_visibility="collapsed",
    )

    st.divider()
    if st.button("ð RafraÃ®chir les donnÃ©es", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
...
if error:
    st.error(f"â Erreur de connexion au webhook n8n : `{error}`")
    st.info("VÃ©rifie que :\n1. `N8N_WEBHOOK_URL` est correct dans `dashboard.py`\n2. Le webhook n8n est actif et retourne du JSON\n3. L'engine a bien envoyÃ© des donnÃ©es rÃ©cemment")
    st.stop()

if df.empty:
    st.warning("â ï¸ Aucune donnÃ©e reÃ§ue. Le payload est vide ou le format n'est pas reconnu.")
    st.stop()

# Colonnes disponibles pour rÃ©fÃ©rence
...
with col_h1:
    st.markdown(f"## {page}")
with col_h2:
    st.caption(f"ð Mis Ã  jour : {fetched_at}")
...
if page == "ð Screener":
    ...
    # ââ Graphique distribution des scores ââ
...
# âââââââââââââââââââââââââââââââââââââââââââââââ
# PAGE 2 â PORTEFEUILLE
# âââââââââââââââââââââââââââââââââââââââââââââââ
elif page == "ð¼ Portefeuille":
    ...
    if port_col:
        ...
        st.info("â¹ï¸ Colonne portefeuille non dÃ©tectÃ©e dans le JSON. Affichage de tous les assets.")
        port_df = df.copy()

    if port_df.empty:
        st.warning("Aucun asset en portefeuille dÃ©tectÃ©.")
    else:
        # KPIs
        ...
        if rec_col:
            ...
            k3.metric("â ï¸ Ã rÃ©duire / sortir", len(exits))
...
# âââââââââââââââââââââââââââââââââââââââââââââââ
# PAGE 3 â DÃTAIL ASSET
# âââââââââââââââââââââââââââââââââââââââââââââââ
elif page == "ð DÃ©tail Asset":
    ...
    selected = st.selectbox("SÃ©lectionne un asset", tickers)

    asset = df[df[ticker_col] == selected].iloc[0]

    # ââ En-tÃªte asset ââ
    col_a, col_b = st.columns([2, 1])
    with col_a:
        name = asset.get("name", selected)
        st.markdown(f"### {name} ({selected})")
        atype = asset.get("asset_type", asset.get("type", "â"))
        st.caption(f"Type : {atype}")

    with col_b:
        rec = asset.get("recommendation", "â")
        color = REC_COLORS.get(rec, "white")
        st.markdown(f"<h2 style='color:{color};text-align:right'>{rec}</h2>", unsafe_allow_html=True)

    st.divider()

    # ââ Scores principaux ââ
    score_display = {
        "ð Score Final":       "final_score",
        "ð¡ Conviction":        "conviction_score",
        "ð Technique":         "technical_score",
        "ð¦ Fondamental":       "fundamental_score",
        "ð MarchÃ©":            "market_score",
        "â ï¸ Risque":            "risk_score",
        "â­ QualitÃ©":           "quality_score",
        "ð Croissance":        "growth_score",
        "ð° Valorisation":      "valuation_score",
        "ð° News":              "news_score",
        "ð® RÃ©gime":            "regime_score",
        "ð§ LiquiditÃ©":         "liquidity_score",
        "ðª Force relative":    "relative_strength_score",
    }
    ...
    if scores_available:
        ...
        st.info("Aucun score trouvÃ© dans le JSON pour cet asset.")

    # ââ Autres donnÃ©es ââ
...
# âââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    # ââ Signaux forts par catÃ©gorie ââ
    categories = {
        "ð¢ BUY CANDIDATE": ("BUY_CANDIDATE", "#00e676"),
        "ð¡ ACCUMULATE":    ("ACCUMULATE",    "#69f0ae"),
        "ð  REDUCE":        ("REDUCE",         "#ff6d00"),
        "ð´ EXIT":          ("EXIT",           "#ff1744"),
        "â« AVOID":         ("AVOID",          "#b71c1c"),
    }
    ...
    # ââ Graphique camembert des recommandations ââ
    st.divider()
    st.subheader("RÃ©partition des recommandations")
